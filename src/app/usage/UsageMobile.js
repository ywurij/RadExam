    ...
    return (
        <main style={{ width: 'min(100%, 760px)', minHeight: '100dvh', margin: '0 auto', padding: 'max(1.25rem, env(safe-area-inset-top)) max(1rem, env(safe-area-inset-right)) max(2rem, env(safe-area-inset-bottom)) max(1rem, env(safe-area-inset-left))', color: '#1e293b', background: '#f8fafc' }}>
            ...
            <header style={{ margin: '1.3rem 0 1.5rem' }}>
                ...
                <p style={{ margin: 0, color: '#64748b', lineHeight: 1.7 }}>iPhone・iPad・Androidで、Mac/PC版の試験データを使って演習するためのアプリです。</p>
            </header>

            <div style={{ display: 'grid', gap: '1rem' }}>
                <section style={sectionStyle}>
                    <h2 style={{ margin: '0 0 0.55rem', fontSize: '1.15rem' }}>1. Mac/PC版でデータを書き出す</h2>
                    <p style={{ margin: 0, color: '#64748b', lineHeight: 1.7 }}>試験データと画像をRadExamバックアップ（.radexam）として書き出し、AirDrop、iCloud Drive、Google Driveなどでモバイル端末へ移します。</p>
                </section>
                <section style={sectionStyle}>
                    <h2 style={{ margin: '0 0 0.55rem', fontSize: '1.15rem' }}>2. モバイル版へ登録する</h2>
                    <p style={{ margin: 0, color: '#64748b', lineHeight: 1.7 }}>ホームの「データ転送」からRadExamバックアップ（.radexam）を選びます。現在の試験と学習履歴は、読み込んだ内容に置き換わります。問題に紐づく参照PDFも引き継がれます。</p>
                </section>
                <section style={sectionStyle}>
