...
- Mac/PC版でアプリを終了・再起動した後も、登録した試験データ、学習履歴、クラウド接続状態、表示設定を保持するよう修正しました。
- モバイル版のOneDrive認証後に接続処理が途切れる問題を修正しました。
- Google Drive／OneDrive接続時の確認処理を並列化し、待ち時間を短縮しました。
- クラウド暗号化の新規設定画面と、パスワード付きバックアップの作成画面を通常UIから外しました。過去の暗号化済みデータは引き続き復元できます。
- Mac/PC版の二重起動を防ぎ、起動中のRadExamを前面へ戻すようにしました。

旧版は起動ごとに異なる保存領域を参照していたため、v0.1.12への更新直後はクラウド同期または`.radexam`バックアップから一度データを復元してください。以後の再起動では端末内に保持されます。
